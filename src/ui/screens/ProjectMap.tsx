...
function matches(node: ProjectNode, query: string): boolean {
  ...
  return (
    <div>
      <button
        ...
        onClick={() => {
          onSelect(node);
          if (isDirectory && !query) setOpen((value) => !value);
        }}
        className={cn(
...
export function ProjectMap() {
  ...
  const root = context.project.structure;
  ...
  return (
    <div className="flex h-screen flex-col">
      ...
      <div className="grid flex-1 grid-cols-[1fr_320px] overflow-hidden">
        ...
        <aside className="overflow-auto bg-surface/40 px-5 py-5">
          {!selected ? (
